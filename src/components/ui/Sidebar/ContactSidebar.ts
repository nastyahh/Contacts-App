...
export function initContactSidebar() {
  ...
  const headerAddContactBtn = document.querySelector(
    ".header__addContact"
  ) as HTMLElement;
  headerAddContactBtn?.addEventListener("click", openSidebar);

  const mainAddContactBtn = document.querySelector(
    ".addContact"
  ) as HTMLElement;
  ...
  if (contactForm) {
    const requiredInputs = contactForm.querySelectorAll<HTMLInputElement>(
